 ...
 *
 * Initialization order (inside bootstrap()):
 ...
 *
 * Delivery contract:
 *   The Frida CLI delivers `-P '{...}'` parameters EXCLUSIVELY by invoking the
 *   agent's `rpc.exports.init(stage, parameters)` method (same mechanism Frida
 *   Gadget uses; see frida-tools/repl.py:308). It does NOT populate
 *   `globalThis.parameters`. Therefore the agent does not auto-run at script
 *   load — it runs when the host calls `init()`. This matches `frida`,
 *   `frida-trace`, and Frida Gadget behavior.
 *
...
/**
 * Bootstrap the agent. Invoked from `rpc.exports.init` once the host
 * delivers `-P` parameters. Throwing here propagates back to the host and
 * surfaces the documented user-facing error contract
 * (`[!] Parameters API rejected: ...`).
 */
function bootstrap(rawParameters: unknown): void {
  // 1. Parameters validation — runs FIRST, before any other code.
  const params = validateParameters(rawParameters);

  // 2. Decoder bit-math self-test — runs at bootstrap before any hook.
  selfTest();
...
// Frida CLI delivers `-P '{...}'` exclusively via this RPC export. See
// frida-tools/repl.py:308 — `script.exports_sync.init(stage, self._user_parameters)`.
// `globalThis.parameters` is NOT populated by the CLI.
rpc.exports = {
  init(_stage: string, parameters: unknown): void {
